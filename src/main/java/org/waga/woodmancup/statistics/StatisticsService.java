...
//		List<Player> members = Arrays.asList( Players.values() ).stream().map(p->p.asPlayer()).collect(Collectors.toList());
...
//	}
//
//	private HallOfFameEntry generateEntry(Player member) {
//		int victories=0;
...
//			WoodmanCupEvent tournament = tournamentRepository.findByTeamId(teamId);
